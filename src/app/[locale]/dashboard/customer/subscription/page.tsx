...
import { CUSTOMER_PLANS } from "@/lib/subscriptions/plans";
import {
...
export default async function CustomerSubscriptionPage({
  ...
  const t = await getTranslations({ locale, namespace: "dashboard.customer.subscription" });

  const user = await requireUser({ allowedRoles: ["customer", "admin"] });

  // Get user's current subscription
  const subscription = await getUserSubscription(user.id, "customer");
