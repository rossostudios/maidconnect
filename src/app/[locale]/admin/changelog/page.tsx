  ...
  return (
    <section className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="font-bold text-3xl text-stone-900">Changelog Management</h1>
          <p className="mt-2 text-stone-600 text-sm">
            Create, edit, and publish sprint updates for your users
          </p>
        </div>
        <Link
          className="flex items-center gap-2 rounded-lg bg-stone-900 px-4 py-2 font-semibold text-sm text-white transition hover:bg-stone-800"
      ...
      {/* Status Filter Tabs */}
      <div className="flex gap-2 border-stone-200 border-b pb-4">
        <Link
          className={`rounded-lg px-4 py-2 font-medium text-sm transition ${
            status
              ? "border border-stone-200 text-stone-600 hover:border-stone-300 hover:text-stone-900"
      ...
      {changelogList.length === 0 ? (
        <div className="rounded-lg border border-stone-200 bg-white p-12 text-center shadow-sm ring-1 ring-black/5">
          <HugeiconsIcon className="mx-auto mb-4 h-12 w-12 text-stone-400" icon={MagicWand01Icon} />
          <h3 className="mb-2 font-bold text-stone-900 text-xl">No Changelogs Yet</h3>
          <p className="mb-6 text-stone-600">Create your first changelog to get started</p>
          <Link
            className="inline-flex items-center gap-2 rounded-lg bg-stone-900 px-6 py-3 font-semibold text-white transition hover:bg-stone-800"
            href="/admin/changelog/new"
          >
            <HugeiconsIcon className="h-4 w-4" icon={Add01Icon} />
            Create Changelog
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1">
                  <div className="mb-3 flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-stone-100 px-3 py-1 font-semibold text-stone-900 text-xs">
                  ...
                  <h2 className="mb-2 font-bold text-stone-900 text-xl">{changelog.title}</h2>

                  {changelog.summary && (
                    <p className="mb-3 line-clamp-2 text-stone-600 text-sm">{changelog.summary}</p>
                  )}
                ...
                <div className="flex flex-col gap-2">
                  <Link
                    className="flex items-center gap-2 rounded-lg border border-stone-200 px-3 py-2 font-medium text-stone-600 text-sm transition hover:border-stone-300 hover:bg-stone-50 hover:text-stone-900"
                  ...
                  <Link
                    className="flex items-center gap-2 rounded-lg bg-stone-900 px-3 py-2 font-medium text-sm text-white transition hover:bg-stone-800"
