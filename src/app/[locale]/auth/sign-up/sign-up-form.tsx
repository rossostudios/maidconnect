...
function getRoleOptionClassName(error: string | undefined): string {
  return cn(
    "flex cursor-pointer flex-col gap-3 rounded-lg border border-stone-300 bg-white p-5 text-sm shadow-sm transition-colors focus-within:border-stone-900 hover:border-stone-400",
...
function RoleSelection({ t, error }: { t: (key: string) => string; error: string | undefined }) {
  return (
    <section className="space-y-5">
      <div className="block font-semibold text-stone-900 text-sm">{t("accountTypeLabel")}</div>
      <p className="text-stone-600 text-xs">{t("accountTypeHelper")}</p>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <label className={getRoleOptionClassName(error)}>
          ...
          <span className="text-stone-600 text-sm">{t("customerDescription")}</span>
        </label>
        <label className={getRoleOptionClassName(error)}>
          <span className="flex items-center gap-2 text-stone-900">
            <input
          ...
          <span className="text-stone-600 text-sm">{t("professionalDescription")}</span>
        </label>
      </div>
      {error ? (
