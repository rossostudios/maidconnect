...
import { createHelpArticle, updateHelpArticle } from "@/app/actions/helpArticles";
...
const propertyCardClass =
  "rounded-3xl border border-stone-200 dark:border-stone-800/80 bg-white dark:bg-stone-950/60 p-5 shadow-[0_1px_2px_rgba(22,22,22,0.05)] backdrop-blur";
const propertyHeadingClass =
  "text-[11px] font-semibold uppercase tracking-[0.2em] text-stone-600 dark:text-stone-400";
const propertyRowClass = "grid grid-cols-[110px_1fr] items-center gap-4 rounded-2xl px-2 py-1.5";
const propertyValueClass =
  "w-full rounded-2xl border border-stone-200 dark:border-stone-800/70 bg-white dark:bg-stone-950 px-3 py-2 text-sm text-stone-900 dark:text-stone-100 outline-none transition focus:border-stone-900 dark:border-stone-100/40 focus:ring-0";
...
function TranslationHealthBadge({
  ...
  if (isEmpty && !otherIsEmpty) {
    ...
    return (
      <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-stone-900 px-2 py-0.5 font-medium text-white text-xs dark:bg-stone-100/10 dark:text-stone-100">
...
export function ArticleForm({ locale, categories, tags, initialData }: ArticleFormProps) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-stone-950">
      {/* Clean Admin Header */}
      <div className="border-stone-200 border-b bg-white px-6 py-4 dark:border-stone-800 dark:bg-stone-950">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              className="text-stone-600 transition-colors hover:text-stone-900 dark:text-stone-100 dark:text-stone-400"
              onClick={() => router.back()}
              type="button"
            >
            ...
            <div>
              <h1 className="font-semibold text-stone-900 text-xl dark:text-stone-100">
                {initialData
                  ? locale === "es"
                    ? "Editar Artículo"
                    : "Edit Article"
          ...
          <div className="flex items-center gap-3">
            <button
              className="flex items-center gap-2 rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm text-stone-600 transition-colors hover:bg-white dark:border-stone-800 dark:bg-stone-950 dark:bg-stone-950 dark:text-stone-400"
            ...
            <button
              className="rounded-lg border border-stone-200 bg-white px-4 py-2 text-sm text-stone-600 transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-50 dark:border-stone-800 dark:bg-stone-950 dark:bg-stone-950 dark:text-stone-400"
              disabled={saving}
              onClick={() => {
                handleSave(false);
              }}
            ...
            <button
              className="rounded-lg bg-stone-900 px-4 py-2 font-medium text-sm text-white transition-colors hover:bg-stone-900 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-stone-100 dark:bg-stone-100 dark:text-stone-950"
              disabled={saving}
              onClick={() => {
                handleSave(true);
              }}
      ...
      {isPreview ? (
        <div className="rounded-lg border border-stone-200 bg-white dark:border-stone-800 dark:bg-stone-950">
          <div className="border-stone-200 border-b bg-white px-6 py-3 dark:border-stone-800 dark:bg-stone-950">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <HugeiconsIcon
                ...
                />
                <div>
                  <p className="font-medium text-sm text-stone-800 dark:text-stone-300">
          ...
          {previewMode === "both" ? (
            ...
            <div className="grid grid-cols-2 divide-x divide-[#f8fafc]">
              ...
              >
                <div className="mb-4 flex items-center gap-2 border-stone-200 border-b pb-2 dark:border-stone-800">
                  <span className="rounded-full bg-[#e2e8f0]/30 px-2 py-0.5 font-medium text-stone-600 text-xs dark:text-stone-400">
                    English
                  </span>
                  {!titleEn && (
                    <span className="flex items-center gap-1 text-stone-900 text-xs dark:text-stone-100">
                      <HugeiconsIcon className="h-3 w-3" icon={Alert02Icon} />
                      Missing title
                    </span>
                  )}
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
              ...
              <Tabs defaultValue="en">
                ...
                <TabsContent value="en">
                  <div className="space-y-8">
                    <div>
                      <input
                        className="w-full border-none px-0 py-2 font-bold text-4xl text-stone-900 placeholder-[#e2e8f0] outline-none dark:text-stone-100"
                        id="title-en"
                    ...
                    <div>
                      <textarea
                        className="w-full resize-none border-none px-0 py-2 text-stone-600 placeholder-[#e2e8f0] outline-none dark:text-stone-400"
                        id="excerpt-en"
            ...
            <div className="space-y-5">
              ...
              <div className={propertyCardClass}>
                ...
                <div className="mt-4 space-y-3 text-sm">
                  <div className={propertyRowClass}>
                    <span className="font-medium text-[12px] text-stone-600 dark:text-stone-400">
                  ...
                  <div className={propertyRowClass}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <button
                        className="rounded-2xl border border-stone-200 px-3 py-1.5 font-medium text-[11px] text-stone-600 transition hover:text-stone-900 dark:border-stone-800/70 dark:text-stone-100 dark:text-stone-400"
              ...
              <div className={propertyCardClass}>
                <div className="flex items-center justify-between">
                  ...
                  {selectedTags.length > 0 && (
                    <span className="rounded-full bg-white px-2 py-0.5 font-medium text-stone-900 text-xs dark:bg-stone-950 dark:text-stone-100">
                      {selectedTags.length}
                    </span>
                  )}
                </div>
                ...
                {selectedTags.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {selectedTags.map((tagId) => {
                      ...
                      return (
                        <span
                          className="inline-flex items-center gap-1 rounded-full bg-[#e2e8f0]/30 px-2 py-1 text-stone-600 text-xs dark:text-stone-400"
                          key={tagId}
                        >
                          <span>{locale === "es" ? tag.name_es : tag.name_en}</span>
                          <button
                            className="text-stone-600 dark:text-stone-400/70"
                            onClick={() => {
                ...
                <div className="mt-3 max-h-48 space-y-1 overflow-y-auto">
                  {tags.length === 0 ? (
                    <p className="text-stone-600 text-xs dark:text-stone-400/70">
                      {locale === "es" ? "No hay etiquetas" : "No tags"}
                    </p>
                  ) : (
                    tags.map((tag) => (
                      <label
                        className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 text-stone-600 text-xs hover:bg-white dark:bg-stone-950 dark:text-stone-400"
                        key={tag.id}
                      >
                        <input
                          checked={selectedTags.includes(tag.id)}
                          className="h-3.5 w-3.5 rounded border-stone-400/40 text-stone-900 focus:ring-0 dark:border-stone-500/40 dark:text-stone-100"
                          onChange={(e) => {
                            if (e.target.checked) {
              ...
              <div className={propertyCardClass}>
                ...
                <div className={cn(propertyRowClass, "mt-4 bg-white dark:bg-stone-950/60")}>
                  <span className="font-medium text-[12px] text-stone-600 dark:text-stone-400">
                    {locale === "es" ? "Estado" : "Status"}
                  </span>
                  <div className="flex items-center justify-between gap-3">
                    <span className="rounded-full bg-white px-3 py-1 font-medium text-sm text-stone-600 dark:bg-stone-950 dark:text-stone-400">
                      {isPublished
                        ? locale === "es"
                          ? "Publicado"
                          : "Published"
                    ...
                    <label className="inline-flex cursor-pointer items-center gap-2">
                      <input
                        checked={isPublished}
                        className="h-5 w-10 cursor-pointer rounded-full border-stone-400/40 text-stone-900 focus:ring-stone-500 dark:border-stone-500/40 dark:text-stone-100 dark:focus:ring-stone-400"
