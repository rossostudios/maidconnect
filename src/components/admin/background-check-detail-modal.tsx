  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/50 p-4 dark:bg-neutral-100/50">
      <div className="relative max-h-[90vh] w-full max-w-4xl overflow-auto border border-neutral-200 bg-white p-8 shadow-2xl dark:border-neutral-800 dark:bg-neutral-950">
        ...
        <RawDataSection rawData={check.rawData} />
...
function ChecksPerformedSection({ checks }: { checks?: string[] | null }) {
...
type RecordSectionProps = { records?: BackgroundCheckResult["results"]["criminal"] };

function CriminalRecordsSection({ records }: RecordSectionProps) {
  if (!records) {
    return null;
  }
      ...
      {records.records.length === 0 ? (
        ...
        <div className="space-y-4">
          {records.records.map((record, index) => (
            <CriminalRecordCard index={index} key={index} record={record} />
          ))}
        </div>
      )}
    </div>
...
function CriminalRecordCard({
  record,
  index,
}: {
  record: NonNullable<BackgroundCheckResult["results"]["criminal"]>["records"][number];
  index: number;
}) {
  return (
...
function IdentityVerificationSection({
  ...
  records?: BackgroundCheckResult["results"]["identity"];
}) {
  if (!records) {
    return null;
  }
...
function RawDataSection({ rawData }: { rawData?: Record<string, unknown> | null }) {
  if (!rawData) {
    return null;
  }
...
function ActionSection({ check, handleApprove, handleReject, isProcessing }: ActionSectionProps) {
