...
export function PriceBreakdown({
  ...
  return (
    <Card className={`border-stone-200 bg-white ${className}`}>
      <CardContent className="space-y-3 p-5">
        <h3 className="font-semibold text-base text-stone-900">Price Breakdown</h3>
        ...
        <div className="space-y-2">
          <div className="flex items-start justify-between text-sm">
            ...
            <span className="font-medium text-stone-900">{formatCOP(baseAmount)}</span>
          </div>

          {addonsTotal > 0 && (
            <div className="flex justify-between text-sm">
              <span className="text-stone-600">Add-ons</span>
              <span className="font-medium text-stone-900">{formatCOP(addonsTotal)}</span>
            </div>
          )}
          ...
          {showPlatformFee && (
            <>
              <div className="flex justify-between pt-2 text-sm">
                ...
                <span className="font-medium text-stone-900">{formatCOP(subtotal)}</span>
              </div>
              ...
              <div className="flex items-start justify-between text-sm">
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="group relative">
                    <HugeiconsIcon
                      className="h-3.5 w-3.5 text-stone-500 transition hover:text-stone-900"
                      icon={HelpCircleIcon}
                    />
                ...
                <span className="font-medium text-stone-900">{formatCOP(platformFee)}</span>
              </div>
            </>
          )}

          <div className="flex justify-between border-stone-200 border-t pt-3 font-semibold text-base">
            <span className="text-stone-900">Total</span>
            <span className="text-stone-900">{formatCOP(totalWithFees)}</span>
          </div>
        </div>

        {showPlatformFee && (
          <Card className="mt-3 border-stone-200 bg-stone-100">
...
export function CompactPrice({
  ...
  if (!showBreakdown) {
    return (
      <div className={`font-semibold text-stone-900 ${className}`}>{formatCOP(hourlyRate)}/hr</div>
    );
  }

  return (
    <div className={className}>
      <div className="font-semibold text-stone-900">{formatCOP(hourlyRate)}/hr</div>
