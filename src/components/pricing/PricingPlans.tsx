...
export function PricingPlans() {
  ...
  if (error) {
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <div className="text-center">
          <p className="mb-4 text-stone-800">Error loading pricing plans</p>
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 md:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            ...
            >
              {plan.highlight_as_popular && (
                <div className="-top-4 -transtone-x-1/2 absolute left-1/2 rounded-full bg-stone-900 px-4 py-1 font-semibold text-sm text-white">
                  Most Popular
                </div>
              )}
                    ...
                    .map((feature, idx) => (
                      <div className="flex items-start gap-3" key={idx}>
                        ...
                        <span className="text-stone-900">
                          {feature.name}
                          {feature.limit && (
                            <span className="ml-1 text-sm text-stone-600">({feature.limit})</span>
                          )}
                        </span>
                      </div>
                    ))}
