...
function isLocationError(message: string): boolean {
  return message.includes("location") || message.includes("Location");
}
...
export function ServiceExecutionCard({ booking, onRatingComplete }: Props) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      const errorMsg = extractErrorMessage(error, "Failed to check in");
      if (isLocationError(errorMsg)) {
  ...
  const handleCheckOut = async (completionNotes?: string) => {
    ...
    try {
      ...
      const errorMsg = extractErrorMessage(error, "Failed to check out");
      if (isLocationError(errorMsg)) {
        setGpsError(errorMsg);
      }
      setMessage({ type: "error", text: errorMsg });
