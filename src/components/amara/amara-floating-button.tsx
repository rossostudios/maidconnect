  ...
  () => import("./amara-chat-interface").then((mod) => mod.AmaraChatInterface),
  {
    ...
    loading: () => (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50">
        <div className="h-12 w-12 animate-spin rounded-full border-4 border-slate-200 border-t-transparent" />
      </div>
    ),
  }
);
    ...
    <>
      ...
      {!isOpen && (
        <div className="fixed right-4 bottom-4 z-50 sm:right-6 sm:bottom-6">
          ...
          {showOnboarding && (
            <div className="amara-pulse-ring absolute inset-0 rounded-full bg-slate-900" />
          )}
          ...
          <button
            ...
            className={cn(
              "relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-slate-900 text-slate-50 shadow-lg transition-all hover:bg-slate-800 hover:shadow-xl active:scale-95 sm:h-16 sm:w-16",
              className
            )}
            onClick={() => {
              setIsOpen(true);
            ...
            type="button"
          >
            <AmaraIcon className="text-slate-50" size={32} />
          </button>
        </div>
      )}
