...
export function ProfileMetricsSkeleton() {
  return (
    <dl aria-label="Loading metrics" className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
      {[1, 2, 3, 4].map((i) => (
        <div
          className="animate-pulse rounded-lg border border-stone-200 bg-white p-6 shadow-sm shadow-sm ring-1 ring-black/5"
          key={i}
        >
    ...
    >
      <div className="animate-pulse space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {[1, 2].map((i) => (
            <div
              className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm ring-1 ring-black/5"
              key={i}
            >
              <div className="space-y-3">
                <div className="h-5 w-2/3 rounded bg-stone-200" />
                <div className="h-4 w-1/2 rounded bg-stone-200" />
                <div className="mt-4 h-10 w-full rounded-full bg-stone-200" />
              </div>
            </div>
          ))}
        </div>
...
export function DocumentsSkeleton() {
  return (
    <ul aria-label="Loading documents" className="divide-y divide-stone-200" role="status">
      {[1, 2, 3].map((i) => (
