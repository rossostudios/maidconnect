...
export function CustomerSearchSection({ professionals }: CustomerSearchSectionProps) {
  ...
  return (
    <section className="border-slate-200 border-t bg-slate-50 py-20 sm:py-24">
      <Container className="space-y-12">
        <div className="max-w-3xl space-y-5">
          <h2 className="font-semibold text-3xl text-slate-900 tracking-tight">
        ...
        {featuredProfessionals.length === 0 ? (
          <Card className="rounded-3xl border-slate-200 bg-slate-50">
            <CardContent className="p-10 text-center text-slate-600 text-sm">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {featuredProfessionals.map((professional) => (
              <Card
                className={cn(
                  "flex h-full flex-col overflow-hidden rounded-3xl border-slate-200 bg-white shadow-sm transition-all duration-300",
              ...
              >
                ...
                <CardContent className="flex flex-1 flex-col gap-3 p-6">
                  <div>
                    <h3 className="font-semibold text-lg text-slate-900">{professional.name}</h3>
                    <p className="text-slate-600 text-sm">
                      {professional.service ?? "Flexible household services"}
                    </p>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 font-semibold text-slate-600 text-xs">
                  ...
                  <p className="text-slate-600 text-sm">{professional.location}</p>
                  <p className="text-slate-600 text-sm">
                    {professional.bio
                  ...
                  <div className="mt-auto flex items-center justify-between text-slate-600 text-sm">
                    <span>{formatCOP(professional.hourlyRateCop) ?? "Rate on request"}</span>
                    <Link
                      className="font-semibold text-slate-900 text-sm underline-offset-4 transition hover:underline"
        ...
        <Card className="rounded-3xl border-slate-200 bg-slate-50/90">
          <CardContent className="flex flex-col gap-4 p-6 text-slate-600 text-sm sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-semibold text-slate-900">Ready to explore more?</p>
              <p>
