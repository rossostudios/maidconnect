...
export function CapabilitiesSection() {
  return (
    <section className="py-16 sm:py-20 lg:py-24" id="capabilities">
      <Container>
        <Card className="rounded-[44px] border-slate-800 bg-slate-900 p-10 text-slate-50 shadow-2xl md:p-14">
          <div className="grid gap-12 lg:grid-cols-[1.05fr_minmax(0,_1fr)]">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {productPillars.map((pillar) => (
                <Card
                  className={cn(
                    "flex flex-col gap-3 rounded-[28px] border-slate-800 bg-slate-900 p-6 text-left transition-colors",
                    "hover:border-slate-700 hover:bg-slate-850"
                  )}
                  key={pillar.title}
                >
                  <HugeiconsIcon className="h-6 w-6 text-slate-400" icon={pillar.icon} />
                  <h3 className="font-semibold text-lg text-slate-50">{pillar.title}</h3>
