      ...
      <Container className="flex flex-col gap-12 md:gap-14">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {useCaseFlows.map((flow) => (
            <Card
              className="flex h-full flex-col rounded-3xl border-slate-200 bg-white shadow-sm"
              key={flow.name}
            >
              <CardHeader className="flex flex-col gap-1">
                <Badge
                  className="w-fit rounded-full bg-slate-900/10 px-2.5 py-1 font-semibold text-slate-900 text-xs uppercase tracking-[0.15em]"
                  variant="secondary"
                >
                  {flow.persona}
                </Badge>
                <h3 className="font-semibold text-slate-900 text-xl">{flow.name}</h3>
              </CardHeader>
              <CardContent className="flex flex-1 flex-col gap-5">
                <p className="text-slate-600 text-sm">{flow.summary}</p>
                <ul className="flex flex-col gap-3 text-sm">
                  {flow.steps.map((step) => (
                    <li className="flex items-start gap-2" key={step}>
                      <span className="mt-1 inline-block h-2 w-2 shrink-0 rounded-full bg-slate-900" />
                      <span>{step}</span>
                    </li>
                  ))}
                </ul>
                <Card className="rounded-2xl border-slate-200 bg-slate-50 px-4 py-3 text-slate-600 text-xs">
