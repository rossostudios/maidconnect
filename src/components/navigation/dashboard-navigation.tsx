...
export function DashboardNavigation({ navLinks, userRole }: Props) {
  ...
  return (
    <>
      ...
      <nav className="hidden items-center gap-6 font-medium text-slate-900 text-sm md:flex">
        {navLinks.map((item) => {
          const active = isActive(item.href);
          return (
            <Link
              className={`relative pb-1 transition hover:text-slate-700 ${
                active
                  ? "font-semibold text-slate-900 after:absolute after:right-0 after:bottom-0 after:left-0 after:h-0.5 after:bg-slate-900 after:content-['']"
                  : ""
              }`}
              href={item.href}
              key={item.href}
        ...
        <Link
          ...
          className={`relative rounded-lg p-1.5 transition hover:bg-slate-100 hover:text-slate-900 ${
            pathname === messagesHref ? "bg-slate-100 text-slate-900" : "text-slate-700"
          }`}
          href={messagesHref}
        >
          <MessageCircle className="h-5 w-5" />
          {unreadCount > 0 && (
            <span className="absolute top-0 right-0 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-slate-900 px-1 font-bold text-[10px] text-white">
        ...
        <button
          ...
          className="relative rounded-lg p-1.5 text-slate-700 transition hover:bg-slate-100 hover:text-slate-900"
        ...
        <Link
          aria-label="Sign out"
          className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-slate-600 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900"
          href="/auth/sign-out"
        >
