...
export function DashboardMobileNav({
  ...
  return (
    <nav className="fixed right-0 bottom-0 left-0 z-40 border-stone-200 border-t bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.05)] md:hidden">
            ...
            return (
              ...
              >
                <div className="relative">
                  <HugeiconsIcon
                    className={`h-6 w-6 ${isItemActive ? "text-stone-900" : "text-stone-600"}`}
                    icon={IconComponent}
                  />
                  {item.badge !== undefined && item.badge > 0 && (
                    <span className="-right-1 -top-1 absolute flex h-5 min-w-[18px] items-center justify-center rounded-full bg-stone-900 px-1.5 font-bold text-[10px] text-white">
                      {item.badge > 9 ? "9+" : item.badge}
                    </span>
                  )}
                </div>
                <span
                  className={`text-xs ${isItemActive ? "font-semibold text-stone-900" : "text-stone-600"}`}
                >
                  {item.label}
                </span>
              </button>
