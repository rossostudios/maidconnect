...
export function StartConversationButton({ bookingId, label = "Message", className }: Props) {
  ...
  return (
    <button
      className={cn(
        "inline-flex items-center gap-2 rounded-lg border border-stone-200 bg-white px-4 py-2 font-semibold text-sm text-stone-700 shadow-sm transition-all hover:border-stone-300 hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      disabled={loading}
      onClick={handleClick}
