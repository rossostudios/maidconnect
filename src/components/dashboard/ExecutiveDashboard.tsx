    ...
    return (
      <div className="space-y-8">
        ...
        <div className="min-h-[60px]">
          <div className="flex items-center gap-4 rounded-lg border border-stone-200 bg-white px-5 py-4 shadow-sm ring-1 ring-black/5">
            <div className="h-5 w-5 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
            <div className="h-5 w-48 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
          </div>
        </div>

        {/* Tabs & Filter skeleton */}
        <div className="flex items-center justify-between">
          <div className="h-10 w-96 animate-pulse rounded-lg bg-stone-50 will-change-[opacity]" />
          <div className="h-10 w-40 animate-pulse rounded-lg bg-stone-50 will-change-[opacity]" />
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            <div className="h-[440px] animate-pulse rounded-lg border border-stone-200 bg-white p-6 shadow-sm ring-1 ring-black/5 will-change-[opacity]">
              <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 animate-pulse rounded-lg bg-stone-50 will-change-[opacity]" />
                  <div className="space-y-2">
                    <div className="h-4 w-32 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
                    <div className="h-8 w-24 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
                  </div>
                </div>
              </div>
              <div className="h-64 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
            </div>
          </div>
            ...
            <div className="space-y-6">
              {[1, 2, 3, 4].map((i) => (
                <div className="space-y-2" key={i}>
                  <div className="h-4 w-32 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
                  <div className="h-8 w-20 animate-pulse rounded bg-stone-50 will-change-[opacity]" />
                </div>
              ))}
            </div>
          </div>
      ...
      {metrics.criticalAlerts.length > 0 ? (
        <div className="space-y-3">
          {metrics.criticalAlerts.map((alert) => (
            <div
              className={cn(
                "flex items-center gap-4 rounded-lg border bg-white px-5 py-4 shadow-sm ring-1 ring-black/5 transition-all",
                alert.type === "critical" && "border-stone-300 bg-stone-100",
                alert.type === "warning" && "border-amber-200 bg-stone-100",
                alert.type === "info" && "border-stone-200 bg-stone-50"
              )}
              key={alert.id}
            >
              <HugeiconsIcon
                className={cn(
                  ...
                  alert.type === "critical" && "text-stone-800",
                  alert.type === "warning" && "text-stone-500",
                  alert.type === "info" && "text-stone-600"
                )}
                icon={Alert01Icon}
              />
              <div className="min-w-0 flex-1">
                <span className="font-semibold text-sm text-stone-900">{alert.title}</span>
                <span className="ml-3 text-sm text-stone-600">{alert.description}</span>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="flex items-center gap-4 rounded-lg border border-stone-200 bg-stone-100 px-5 py-4 shadow-sm ring-1 ring-black/5">
          <HugeiconsIcon
            className="h-5 w-5 flex-shrink-0 text-stone-700"
      ...
      <Tabs className="w-full" onValueChange={setActiveTab} value={activeTab}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <HugeiconsIcon className="h-5 w-5 text-stone-500" icon={Calendar03Icon} />
        ...
        <TabsContent value="overview">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <Card className="border-stone-200 bg-white shadow-sm ring-1 ring-black/5 transition-shadow hover:shadow-lg lg:col-span-2">
              <CardHeader className="flex flex-row items-center justify-between pb-8">
                <div className="flex items-center gap-4">
                  <div className="rounded-lg bg-stone-600/10 p-3">
                    <HugeiconsIcon className="h-7 w-7 text-stone-600" icon={DollarCircleIcon} />
                  </div>
                  <div>
                    <p className="mb-1 font-semibold text-stone-500 text-xs uppercase tracking-wider">
                      Revenue Trend
                    </p>
                    <p className="font-bold text-3xl text-stone-900">
                      ${(metrics.todayRevenue / 100).toLocaleString()}
                    </p>
                    <p className="mt-1 text-sm text-stone-500">Today's revenue</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    aria-label="Edit revenue settings"
                    className="rounded-lg bg-stone-50 p-2 transition-colors hover:bg-stone-200"
                    onClick={() => alert("Edit revenue tracking settings - Feature coming soon!")}
                  >
                    <HugeiconsIcon className="h-4 w-4 text-stone-500" icon={Edit02Icon} />
                  </button>
                  <button
                    ...
                    <AreaChart data={metrics.revenueData}>
                      <defs>
                        <linearGradient id="colorRevenue" x1="0" x2="0" y1="0" y2="1">
                          <stop offset="5%" stopColor="stone-600" stopOpacity={0.1} />
                          <stop offset="95%" stopColor="stone-600" stopOpacity={0} />
                        </linearGradient>
                      </defs>
            ...
            <div className="space-y-6">
              {/* Pending Bookings */}
              <Card className="border-stone-200 bg-white shadow-sm ring-1 ring-black/5 transition-shadow hover:shadow-lg">
                <CardContent className="p-8">
                  <div className="mb-6 flex items-center gap-3">
                    <div className="rounded-lg bg-stone-600/5 p-3">
                      <HugeiconsIcon className="h-6 w-6 text-stone-600" icon={TimeScheduleIcon} />
                    </div>
                    <p className="font-semibold text-stone-500 text-xs uppercase tracking-wider">
                      Pending Bookings
                    </p>
                  </div>
                  <p className="mb-2 font-bold text-4xl text-stone-900">
                    {metrics.pendingBookings}
                  </p>
                  <p className="text-sm text-stone-500">Awaiting acceptance</p>
                </CardContent>
              </Card>
              ...
              <Card className="border-stone-200 bg-white shadow-sm ring-1 ring-black/5 transition-shadow hover:shadow-lg">
                <CardContent className="p-8">
                  ...
                  <p className="mb-2 font-bold text-4xl text-stone-900">{metrics.activeBookings}</p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <Card className="border-stone-200 bg-white shadow-sm ring-1 ring-black/5 transition-shadow hover:shadow-lg">
              <CardHeader className="flex flex-row items-center justify-between pb-6">
                <div className="flex items-center gap-4">
                  <div className="rounded-lg bg-stone-900/10 p-3">
                    <HugeiconsIcon className="h-6 w-6 text-stone-900" icon={UserMultiple02Icon} />
                  </div>
                  <div>
                    <p className="mb-1 font-semibold text-stone-500 text-xs uppercase tracking-wider">
            ...
            <Card className="border-stone-200 bg-white shadow-sm ring-1 ring-black/5 transition-shadow hover:shadow-lg">
              <CardHeader className="flex flex-row items-center justify-between pb-6">
                <div>
                  ...
                  <p className="text-sm text-stone-500">Conversion pipeline</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                ...
                <div className="h-48">
                  <ResponsiveContainer height="100%" width="100%">
                    <BarChart data={metrics.bookingFunnelData} layout="vertical">
                      <CartesianGrid horizontal={false} stroke="stone-200" strokeDasharray="3 3" />
                      <XAxis
                        axisLine={false}
                        fontSize={12}
                        stroke="stone-500"
                      ...
                      <YAxis
                        ...
                        type="category"
                      />
                      <Bar dataKey="count" fill="stone-600" radius={[0, 8, 8, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
        ...
        <TabsContent value="revenue">
          <div className="py-20 text-center">
            <p className="text-stone-500">Revenue analytics coming soon...</p>
          </div>
        </TabsContent>
        ...
        <TabsContent value="supply">
          <div className="py-20 text-center">
            <p className="text-stone-500">Supply management coming soon...</p>
          </div>
        </TabsContent>
        ...
        <TabsContent value="bookings">
          <div className="py-20 text-center">
            <p className="text-stone-500">Booking analytics coming soon...</p>
          </div>
        </TabsContent>
      </Tabs>
    </div>
