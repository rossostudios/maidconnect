...
export function DateRangePicker({ value, onChange, placeholder, name, required }: DateRangePickerProps) {
                    ...
                    <div className="grid grid-cols-7 gap-1">
                        {calendarDays.map(({ date, inCurrentMonth }) => {
                            ...
                            return (
                                <button
                                    className={cn(
                                        "flex h-9 w-9 items-center justify-center text-sm transition-all relative",
                                        geistSans.className,
                                        inCurrentMonth ? "text-neutral-900" : "text-neutral-300",
                                        !inCurrentMonth && "cursor-default",
                                        // Range styling
                                        (start || end) && "bg-orange-500 text-white z-10 rounded-lg shadow-sm hover:bg-orange-600",
                                        inRange && "bg-orange-50 text-orange-900 rounded-none",
                                        start && value.to && "rounded-r-none",
                                        end && "rounded-l-none",
                                        !selected && isToday && "border border-orange-500 font-medium text-orange-600 rounded-lg",
                                        !selected && !isToday && inCurrentMonth && "hover:bg-neutral-100 rounded-lg"
                                    )}
