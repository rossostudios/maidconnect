    ...
    <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-6 shadow-sm">
      ...
      <div className="mb-4 grid gap-4" style={{ gridTemplateColumns: `repeat(${columns}, 1fr)` }}>
        {Array.from({ length: columns }).map((_, i) => (
          <Skeleton className="h-6 w-full rounded" key={`header-${i}`} />
        ))}
      </div>
      {/* Rows */}
      <div className="space-y-3">
        {Array.from({ length: rows }).map((_, rowIndex) => (
          ...
          >
            {Array.from({ length: columns }).map((__, colIndex) => (
              <Skeleton className="h-8 w-full rounded" key={`cell-${rowIndex}-${colIndex}`} />
            ))}
          </div>
        ))}
      </div>
    ...
    <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-6 shadow-sm">
      ...
      <div className="mb-3 grid grid-cols-7 gap-2">
        {Array.from({ length: 7 }).map((_, i) => (
          <Skeleton className="h-8 w-full rounded" key={`day-${i}`} />
        ))}
      </div>
      {/* Calendar grid */}
