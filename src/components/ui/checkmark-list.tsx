...
export function CheckmarkList({ children, className, variant = "default" }: CheckmarkListProps) {
  const spacing = {
    ...
    compact: "space-y-2",
    large: "space-y-6",
    savvycal: "divide-y divide-stone-900/10",
  };
...
export function CheckmarkListItem({
          ...
          <div className="flex flex-1 items-start gap-4">
            <div
              className={cn(
                ...
                negative ? "bg-red-500" : "bg-green-500"
              )}
            >
              <HugeiconsIcon className="h-5 w-5 text-white" icon={Tick02Icon} strokeWidth={3} />
            </div>
            <h3 className="serif-headline-md pt-0.5 text-stone-900">{title}</h3>
          </div>

          {/* Right: Description */}
          <div className="flex-1 pl-12 md:pl-0">
            <p className="text-stone-900/70 leading-relaxed">{children}</p>
          </div>
        </div>
      </li>
    );
  ...
  return (
    <li className={cn("flex items-start gap-3", className)}>
      ...
      >
        <HugeiconsIcon
          className={cn("h-3.5 w-3.5", negative ? "text-red-600" : "text-green-600")}
          icon={Tick02Icon}
          strokeWidth={3}
        />
      </div>
      <div className="flex-1">
        <span className="text-base text-stone-900 leading-relaxed">{children}</span>
        {description && (
          <p className="mt-1 text-sm text-stone-900/60 leading-relaxed">{description}</p>
        )}
      </div>
    </li>
  );
