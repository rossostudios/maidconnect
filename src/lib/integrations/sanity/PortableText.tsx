...
export const portableTextComponents: PortableTextComponents = {
  block: {
    ...
    // Paragraph
    normal: ({ children }) => (
      <p className="text-neutral-400 leading-7 [&:not(:first-child)]:mt-4">{children}</p>
    ),
    // Blockquote
    blockquote: ({ children }) => (
      <blockquote className="mt-6 border-orange border-l-4 pl-6 text-neutral-400 italic">
        {children}
      </blockquote>
    ),
  },
  ...
  list: {
    // Bulleted list
    bullet: ({ children }) => (
      <ul className="my-6 ml-6 list-disc text-neutral-400 [&>li]:mt-2">{children}</ul>
    ),
    // Numbered list
    number: ({ children }) => (
      <ol className="my-6 ml-6 list-decimal text-neutral-400 [&>li]:mt-2">{children}</ol>
    ),
  },
  ...
  marks: {
    ...
    // Code
    code: ({ children }) => (
      <code className="relative rounded bg-grey/30 px-[0.3rem] py-[0.2rem] font-mono font-semibold text-midnight text-sm">
        {children}
      </code>
    ),
    // Underline
    ...
    link: ({ children, value }) => {
      ...
      if (href.startsWith("http")) {
        return (
          <a
            className="font-medium text-orange underline underline-offset-4 hover:text-orange"
            href={href}
  ...
  types: {
    ...
    image: ({ value }) => {
      ...
      return (
        <figure className="my-8">
          <div className="relative aspect-video overflow-hidden rounded-lg border border-grey">
            <Image
              alt={alt}
    ...
    code: ({ value }) => {
      ...
      return (
        <div className="my-6">
          {filename && (
            <div className="rounded-t-lg bg-midnight px-4 py-2 font-mono text-neutral-400/50 text-sm">
              {filename}
            </div>
          )}
          <pre
          ...
          >
            <code className={`font-mono text-silver text-sm language-${language || "plaintext"}`}>
              {code}
            </code>
          </pre>
        </div>
