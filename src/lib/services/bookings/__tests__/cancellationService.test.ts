...
const mockStripe = {
  paymentIntents: {
    retrieve: mock(() => Promise.resolve({ id: "pi_123", status: "requires_capture" })),
...
describe("Cancellation Service - Policy Validation", () => {
  ...
  describe("validateCancellationPolicy", () => {
    it("should allow cancellation when more than 24 hours before service", () => {
      ...
      const result = validateCancellationPolicy(futureDate.toISOString(), "confirmed");
    ...
    it("should reject cancellation for past services", () => {
      ...
      const result = validateCancellationPolicy(pastDate.toISOString(), "confirmed");
    ...
    it("should reject cancellation for completed bookings", () => {
      ...
      const result = validateCancellationPolicy(futureDate.toISOString(), "completed");
...
describe("Cancellation Service - Stripe Refund Processing", () => {
  ...
  describe("processStripeRefund", () => {
    it("should return success when no payment intent provided", async () => {
      const result = await processStripeRefund(null, 5_000_000);
    ...
    it("should cancel payment intent with requires_capture status", async () => {
      ...
      const result = await processStripeRefund("pi_123", 5_000_000);
    ...
    it("should refund payment intent with succeeded status", async () => {
      const mockRetrieve = mock(() => Promise.resolve({ id: "pi_456", status: "succeeded" }));
      const mockRefundCreate = mock(() => Promise.resolve({ id: "re_123", status: "succeeded" }));
      ...
      const result = await processStripeRefund("pi_456", 2_500_000);
    ...
    it("should skip refund when amount is zero", async () => {
      const mockRetrieve = mock(() => Promise.resolve({ id: "pi_789", status: "succeeded" }));
    ...
    it("should handle Stripe retrieval errors", async () => {
      const mockRetrieve = mock(() => Promise.reject(new Error("Payment intent not found")));
      ...
      const result = await processStripeRefund("pi_invalid", 1_000_000);
    ...
    it("should handle Stripe cancellation errors", async () => {
      ...
      const mockCancel = mock(() => Promise.reject(new Error("Already canceled")));
      ...
      const result = await processStripeRefund("pi_999", 1_000_000);
    ...
    it("should handle Stripe refund creation errors", async () => {
      const mockRetrieve = mock(() => Promise.resolve({ id: "pi_888", status: "succeeded" }));
      const mockRefundCreate = mock(() => Promise.reject(new Error("Insufficient funds")));
      ...
      const result = await processStripeRefund("pi_888", 5_000_000);
    ...
    it("should handle non-Error exceptions", async () => {
      ...
      const result = await processStripeRefund("pi_777", 1_000_000);
  ...
  describe("cancelBookingInDatabase", () => {
    it("should successfully cancel booking in database", async () => {
      ...
      const mockUpdate = mock(() => ({
        eq: mock(() => ({
          select: mock(() => ({
            single: mock(() => Promise.resolve({ data: mockBooking, error: null })),
          })),
        })),
      }));
    ...
    it("should use default reason when not provided", async () => {
      ...
      const result = await cancelBookingInDatabase(supabase, "booking-456", "user-789");
    ...
    it("should handle null booking data", async () => {
      ...
      const result = await cancelBookingInDatabase(supabase, "booking-888", "user-456");
...
describe("Cancellation Service - Notification Handling", () => {
  describe("prepareCancellationNotificationData", () => {
    it("should prepare notification data with all fields", () => {
      ...
      const result = prepareCancellationNotificationData(booking, "John Doe", "Jane Smith");
    ...
    it("should include duration and address as TBD", () => {
      ...
      const result = prepareCancellationNotificationData(booking, "Customer", "Professional");
...
describe("Cancellation Service - Formatting Utilities", () => {
  describe("formatRefundAmount", () => {
    it("should format refund amount in COP", () => {
      const result = formatRefundAmount(5_000_000, "cop");
    ...
    it("should format refund amount in USD", () => {
      const result = formatRefundAmount(10_000, "usd");

      expect(result).toContain("100");
    });

    it("should default to COP when currency is null", () => {
      const result = formatRefundAmount(2_000_000, null);
