...
describe('BalanceService', () => {
  ...
  describe('calculateProfessionalEarnings', () => {
    it('should deduct 18% platform fee from booking amount', () => {
      const bookingAmount = 100000; // 100,000 COP
      const expectedEarnings = 82000; // 82,000 COP (after 18% fee)
    ...
    it('should round earnings to nearest integer', () => {
      const bookingAmount = 100001; // Results in 82,000.82
      const expectedEarnings = 82001; // Rounded
    ...
    it('should calculate correctly for typical booking amounts', () => {
      ...
      expect(service.calculateProfessionalEarnings(60000)).toBe(49200);

      // Typical 8-hour deep clean: 120,000 COP
      expect(service.calculateProfessionalEarnings(120000)).toBe(98400);

      // Large booking: 500,000 COP
      expect(service.calculateProfessionalEarnings(500000)).toBe(410000);
    });
  });
    ...
    it('should parse numeric setting value', async () => {
      ...
      mockSupabase.from = vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
            single: vi.fn(() =>
              Promise.resolve({ data: { setting_value: 2.0 }, error: null })
            ),
          })),
        })),
      }));
    ...
    it('should parse string setting value', async () => {
      ...
      mockSupabase.from = vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
            single: vi.fn(() =>
              Promise.resolve({ data: { setting_value: '1.75' }, error: null })
            ),
          })),
        })),
      }));
    ...
    it('should parse numeric setting value', async () => {
      ...
      mockSupabase.from = vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
            single: vi.fn(() =>
              Promise.resolve({ data: { setting_value: 100000 }, error: null })
            ),
          })),
        })),
      }));
  ...
  describe('validateInstantPayout', () => {
    it('should reject payout below minimum threshold', async () => {
        ...
        if (table === 'payout_rate_limits') {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  single: vi.fn(() => Promise.resolve({ data: { instant_payout_count: 0 }, error: null })),
                })),
              })),
            })),
          };
        ...
        if (table === 'platform_settings') {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(() =>
                  Promise.resolve({ data: { setting_value: 50000 }, error: null })
                ),
              })),
            })),
          };
      ...
      const result = await service.validateInstantPayout('prof-123', 30000); // Below 50k minimum

      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(expect.stringContaining('Minimum instant payout is 50,000 COP'));
    });
    ...
    it('should reject payout if available balance is insufficient', async () => {
            ...
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                single: vi.fn(() =>
                  Promise.resolve({
                    data: {
                      available_balance_cop: 40000, // Only 40k available
                      instant_payout_enabled: true,
                      stripe_connect_account_id: 'acct_123',
                      stripe_connect_onboarding_status: 'complete',
                    },
      ...
      const result = await service.validateInstantPayout('prof-123', 60000); // Requesting 60k
    ...
    it('should reject if Stripe Connect is not set up', async () => {
      ...
      const result = await service.validateInstantPayout('prof-123', 60000);

      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        expect.stringContaining('Please complete your payout setup')
      );
    });
    ...
    it('should calculate fee correctly (1.5% default)', async () => {
              ...
              eq: vi.fn((key: string) => ({
                single: vi.fn(() => {
                  ...
                  if (key === 'minimum_instant_payout_cop') {
                    return Promise.resolve({ data: { setting_value: 50000 }, error: null });
                  }
      ...
      const requestedAmount = 100000; // 100,000 COP
      const expectedFee = 1500; // 1.5% = 1,500 COP
      const expectedNet = 98500; // 98,500 COP
    ...
    it('should pass validation for valid payout request', async () => {
        ...
        if (table === 'payout_rate_limits') {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  single: vi.fn(() => Promise.resolve({ data: { instant_payout_count: 1 }, error: null })),
                })),
              })),
            })),
          };
      ...
      const result = await service.validateInstantPayout('prof-123', 100000);
