...
test.describe("Amara Professional Search (V2 Generative UI)", () => {
  ...
  test.describe("Professional Search Flow", () => {
    test("should search for professionals and display results", async ({ page }) => {
      ...
      await expect(professionalList).toBeVisible({ timeout: 10_000 });
  ...
  test.describe("Availability Selector Flow", () => {
    ...
    test("should allow date and time selection", async ({ page }) => {
      // NOTE: Authentication handled by auth.setup.ts
      ...
      const availableDate = page.locator('button[class*="available"]:not([disabled])').first();
  ...
  test.describe("Booking Summary Flow", () => {
    test("should display booking summary after date/time selection", async ({ page }) => {
      // NOTE: Authentication handled by auth.setup.ts
      ...
      await expect(bookingSummary).toBeVisible({ timeout: 10_000 });
