...
import type { Professional, ProfessionalSearchParams, ProfessionalListResponse } from '@/types/api/professional';
 ...
 */
export async function getFeaturedProfessionals(limit: number = 10): Promise<Professional[]> {
  try {
    const { data, error } = await supabase
      .from('professional_profiles')
      .select('*')
