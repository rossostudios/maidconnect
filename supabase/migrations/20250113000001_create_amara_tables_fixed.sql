-- =============================================
-- Amara AI Chat System Database Schema (Fixed)
-- =============================================
-- Description: Creates tables for Amara AI assistant conversations and messages
-- Fixed: Uses IF NOT EXISTS to handle already-existing objects
-- Created: 2025-01-13
-- =============================================
...
CREATE INDEX IF NOT EXISTS idx_amara_conversations_user_id ON public.amara_conversations(user_id);
CREATE INDEX IF NOT EXISTS idx_amara_conversations_active ON public.amara_conversations(user_id, is_active) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_amara_conversations_last_message ON public.amara_conversations(user_id, last_message_at DESC);

-- Performance indexes for amara_messages
CREATE INDEX IF NOT EXISTS idx_amara_messages_conversation_id ON public.amara_messages(conversation_id);
CREATE INDEX IF NOT EXISTS idx_amara_messages_created_at ON public.amara_messages(conversation_id, created_at ASC);
CREATE INDEX IF NOT EXISTS idx_amara_messages_role ON public.amara_messages(role);
