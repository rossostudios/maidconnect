...
-- Description: Messaging system for customer-professional communication
-- Author: Claude Code
-- Date: 2025-11-06
...
CREATE POLICY "Users can view messages in their conversations"
  ...
  USING (
    EXISTS (
      ...
      AND (conversations.customer_id = (SELECT auth.uid()) OR conversations.professional_id = (SELECT auth.uid()))
    )
  );
...
BEGIN
  ...
  IF OLD.read_at IS NULL AND NEW.read_at IS NOT NULL THEN
    ...
    IF (SELECT auth.uid()) = conv_customer_id THEN
