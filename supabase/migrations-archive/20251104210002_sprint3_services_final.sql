-- Sprint 3: Service Management - FINAL FIX
-- Fixed: RLS policies now use direct column references instead of table-qualified names
...
CREATE POLICY "Professionals can manage their own pricing tiers"
  ...
  USING (
    service_id IN (
      SELECT id FROM professional_services WHERE profile_id = auth.uid()
    )
  );
