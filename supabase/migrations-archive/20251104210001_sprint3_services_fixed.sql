...
CREATE INDEX IF NOT EXISTS idx_professional_services_profile ON professional_services (profile_id);
CREATE INDEX IF NOT EXISTS idx_professional_services_category ON professional_services (category_id) WHERE category_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_professional_services_active ON professional_services (is_active, is_featured) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_professional_services_type ON professional_services (service_type);
CREATE INDEX IF NOT EXISTS idx_professional_services_rating ON professional_services (average_rating DESC) WHERE average_rating > 0;

ALTER TABLE professional_services ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Public can view active services" ON professional_services;
CREATE POLICY "Public can view active services"
  ON professional_services FOR SELECT USING (is_active = true);
...
CREATE INDEX IF NOT EXISTS idx_pricing_tiers_active ON service_pricing_tiers (is_active, tier_level) WHERE is_active = true;
...
CREATE POLICY "Professionals can manage their own pricing tiers"
  ...
  USING (
    EXISTS (
      ...
      WHERE ps.id = service_pricing_tiers.service_id
...
CREATE INDEX IF NOT EXISTS idx_service_addons_active ON service_addons (is_active, display_order) WHERE is_active = true;
...
BEGIN
  ...
  IF tier_id_param IS NOT NULL THEN
    ...
    WHERE spt.id = tier_id_param
    AND spt.service_id = service_id_param;
  END IF;
  ...
  IF array_length(addon_ids_param, 1) > 0 THEN
    ...
    WHERE sa.id = ANY(addon_ids_param)
    AND sa.service_id = service_id_param;
  END IF;
