...
async function migrateTags() {
	...
	try {
		...
		const existingTags = await sanityClient.fetch(
			`*[_type == "helpTag"] { _id, slug }`,
		);
		...
		for (const tag of tags as HelpTag[]) {
			const exists = existingTags.find((t: { slug: { current: string } }) =>
				t.slug.current === tag.slug
			);
...
function printDefaultTags() {
	const defaultTags = [
		{ slug: "getting-started", nameEn: "Getting Started", nameEs: "Primeros Pasos", color: "#3B82F6" },
		{ slug: "payment", nameEn: "Payment", nameEs: "Pago", color: "#10B981" },
		{ slug: "troubleshooting", nameEn: "Troubleshooting", nameEs: "Solución de Problemas", color: "#EF4444" },
		...
		{ slug: "professional", nameEn: "For Professionals", nameEs: "Para Profesionales", color: "#14B8A6" },
