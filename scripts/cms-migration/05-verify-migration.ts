...
function printSummary() {
  ...
  // Print table
  console.log(
    "| Section           | Supabase | Sanity | Status       | Message                          |"
  );
  console.log(
    "|-------------------|----------|--------|--------------|----------------------------------|"
  );
  ...
  for (const result of results) {
    const supabase =
      result.supabaseCount !== null ? result.supabaseCount.toString().padEnd(8) : "N/A     ";
    ...
    console.log(
      `| ${result.section.padEnd(17)} | ${supabase} | ${sanity} | ${status} | ${message} |`
    );
  }
